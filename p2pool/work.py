...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def freshen_addresses(self, c):
        ...
        self.address = yield deferral.retry('Error getting a dynamic address from the block data source:', 5)(lambda: self.bitcoind.rpc_getnewaddress('p2pool'))()
    ...
    def preprocess_request(self, user):
        ...
        if time.time() > self.current_work.value['last_update'] + 60:
            raise jsonrpc.Error_for_code(-12345)(u'lost contact with the block data source')
            ...
            try:
                if pow_hash <= header['bits'].target or p2pool.DEBUG:
                    ...
                    if pow_hash <= header['bits'].target:
                        print
                        print 'GOT BLOCK FROM MINER! Passing to the block data source! %s%064x' % (self.node.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, header_hash)
                        print
            except:
